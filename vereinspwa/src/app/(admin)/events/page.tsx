  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {filtered.length} {filtered.length === 1 ? 'Termin' : 'Termine'}
          </p>
        </div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit mb-4">
        {([
          { key: 'upcoming' as ViewMode, label: 'Kommende' },
          { key: 'past' as ViewMode, label: 'Vergangene' },
          { key: 'all' as ViewMode, label: 'Alle' },
        ]).map(tab => (
          <button
            key={tab.key}
            onClick={() => setViewMode(tab.key)}
            className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${
              viewMode === tab.key ? 'bg-white shadow text-gray-900' : 'text-gray-500'
            }`}
          >
            {tab.label}
          </button>
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <Badge variant="destructive" className="text-xs bg-red-100 text-red-700 border-0">
                      {event.responseCount?.declined ?? 0}
                    </Badge>
                    <Badge variant="muted" className="text-xs">
                      {eventPlayers.length - (event.responseCount?.total ?? 0)}
                    </Badge>
                  </div>
      ...
      <Dialog open={!!cancelTarget} onOpenChange={(o) => !o && setCancelTarget(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-gray-600">
            <strong>{cancelTarget?.title}</strong> wird abgesagt. Alle betroffenen Spieler
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="z.B. Platz gesperrt"
            />
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-gray-600">
            <strong>{deleteTarget?.title}</strong> wird unwiderruflich gelöscht.
          </p>
          <DialogFooter>
