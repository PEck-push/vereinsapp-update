  ...
  async function handleSave() {
    ...
    try {
      await setDoc(
        ...
        { name: clubName, primaryColor },
        { merge: true }
      )
      // Apply color as CSS variable immediately for instant preview
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <CalendarLinkRow
          label="Alle Termine"
          url={`${appUrl}/api/ical?clubId=${CLUB_ID}`}
          copied={copied === 'all'}
          onCopy={() => copy(`${appUrl}/api/ical?clubId=${CLUB_ID}`, 'all')}
        />
        {teams.map(team => (
          <CalendarLinkRow
            key={team.id}
            label={team.name}
            url={`${appUrl}/api/ical?clubId=${CLUB_ID}&teamId=${team.id}`}
            copied={copied === team.id}
            onCopy={() => copy(`${appUrl}/api/ical?clubId=${CLUB_ID}&teamId=${team.id}`, team.id)}
            color={team.color}
          />
        ))}
      </div>
...
function CalendarLinkRow({
  label, url, copied, onCopy, color,
}: {
  label: string; url: string; copied: boolean; onCopy: () => void; color?: string
}) {
  return (
...
function SeasonSection() {
  ...
  const [startMonth, setStartMonth] = useState(6) // July (0-indexed)
  const [saving, setSaving] = useState(false)
    ...
    <section className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <select
          ...
          className="flex h-10 w-48 items-center justify-between rounded-[6px] border border-input bg-background px-3 py-2 text-sm"
        >
