 ...
 *
 * Imports match schedule from ÖFB/ÖFBL pages.
 *
 * Two modes:
 * 1. URL mode: { teamId, oefblUrl, clubName } — fetches the page server-side
 * 2. HTML mode: { teamId, html, clubName }     — parses user-pasted HTML directly
 *
 * HTML mode is the fallback when oefb.at blocks automated requests (403).
 *
 * Returns: { imported, skipped, errors[], totalParsed }
 * Security: Admin-only.
 */
...
function parseOefblHtml(html: string): ParsedMatch[] {
  ...
  $('table tbody tr, table tr').each((_i, el) => {
    ...
    // Date+Time combined in first cell (e.g. "Sa 05.04.2025 15:00")
    if (!dateStr && texts[0]) {
    ...
    // Find team matchup: "Team A - Team B"
    for (const text of texts) {
      if (homeTeam) break
    ...
    // Standard column layout: Date, Time, Home, Away, Location
    ...
    // cells[0]=Date/Time, cells[1]=Matchup, cells[2]=Location/Result
  ...
  if (matches.length === 0) {
    ...
    const patterns = [
      /(\d{2}\.\d{2}\.\d{2,4})\s+(\d{1,2}:\d{2})\s+(.{3,40}?)\s*[-–]\s*(.{3,40}?)(?:\s+\d+:\d+)?(?:\n|$)/g,
      /(\d{2}\.\d{2}\.\d{2,4})\s*[,\s]+(\d{1,2}:\d{2})?\s*[,\s]*(.{3,40}?)\s*[-–:]\s*(.{3,40}?)(?:\n|$)/g,
    ]
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { teamId, oefblUrl, clubName, html: pastedHtml } = body

    if (!teamId) {
      return NextResponse.json({ error: 'teamId benötigt' }, { status: 400 })
    }

    let html: string

    if (pastedHtml && typeof pastedHtml === 'string' && pastedHtml.length > 50) {
      // ── HTML paste mode ──
      html = pastedHtml
    } else if (oefblUrl) {
      // ── URL fetch mode ──
      ...
      try {
        ...
        if (!res.ok) {
          return NextResponse.json(
            {
              error: `ÖFB-Seite antwortet mit HTTP ${res.status}. Bitte verwende stattdessen den manuellen HTML-Import.`,
              useHtmlPaste: true,
            },
            { status: 502 }
          )
        ...
        return NextResponse.json(
          {
            error: 'ÖFB-Seite nicht erreichbar. Bitte verwende stattdessen den manuellen HTML-Import.',
            useHtmlPaste: true,
          },
          { status: 502 }
        )
      ...
      return NextResponse.json({ error: 'oefblUrl oder html benötigt' }, { status: 400 })
    }

    // Parse matches
    const parsedMatches = parseOefblHtml(html)
      ...
      return NextResponse.json(
        {
          ...
          debug: {
            ...
            hint: pastedHtml
              ? 'Der eingefügte HTML-Code enthält keine erkennbaren Spielpläne. Bitte stelle sicher, dass du den kompletten Seitenquelltext einfügst (Rechtsklick → "Seitenquelltext anzeigen").'
              : 'Die Seite verwendet möglicherweise JavaScript-Rendering. Bitte verwende den manuellen HTML-Import.',
          },
        },
        { status: 422 }
      )
