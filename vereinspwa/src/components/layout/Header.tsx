...
export function Header({ clubName = 'Vereinsname', logoUrl, userEmail, onMenuToggle }: HeaderProps) {
    ...
    <header className="h-14 flex items-center justify-between px-4 border-b bg-white shrink-0" style={{ borderColor: '#e9ecef' }}>
      ...
      <div className="flex items-center gap-3 md:hidden">
        ...
        <div className="flex items-center gap-2">
          {/* Logo with primary color behind transparent areas */}
          <div
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="flex items-center gap-2 h-9 px-2 hover:bg-gray-100" style={{ borderRadius: '6px' }}>
            ...
            <span className="hidden sm:block text-sm text-gray-700 max-w-[160px] truncate">{userEmail ?? 'Admin'}</span>
            <ChevronDown className="w-3.5 h-3.5 text-gray-400" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuItem disabled><User className="w-4 h-4 mr-2" /><span className="text-xs text-gray-500 truncate">{userEmail}</span></DropdownMenuItem>
